...
4.  **Channel Features**:
    ...
    -   **`KICK` Command**: Channel operators can remove users from a channel.
    -   **`INVITE` Command**: Channel operators can invite other users to a channel.
...
## Next Steps
-   Implement `TOPIC` and `MODE` commands.
